...
function pdvwc_add_admin_menu() {
	add_submenu_page(
		...
		'manage_options',
		'pdvwc-settings',
		'pdvwc_settings_page_html'
	);
}
add_action( 'admin_menu', 'pdvwc_add_admin_menu' );

// Helper to add checkbox field.
function pdvwc_add_checkbox_field( $id, $title, $description ) {
	add_settings_field(
		$id,
		$title,
		'pdvwc_checkbox_field_callback',
		'pdvwc-options',
		'pdvwc_button_style_section',
		array(
			'label_for'   => $id,
...
function pdvwc_add_color_field( $id, $title, $description ) {
	add_settings_field(
		$id,
		$title,
		'pdvwc_color_field_callback',
		'pdvwc-options',
		'pdvwc_button_style_section',
		array(
			'label_for'   => $id,
...
function pdvwc_settings_page_html() {
	if ( ! current_user_can( 'manage_options' ) ) {
		return;
	}

	// A hidden "0" input before each checkbox ensures an unchecked box still
	// submits a value — otherwise the Settings API leaves the stored option
	// untouched when a checkbox is simply missing from the POST body, so
	// unchecking a box would never actually save as "off".
	?>
	<div class="wrap">
		<h1><?php esc_html_e( 'Document Viewer Settings', 'product-document-viewer-for-woocommerce' ); ?></h1>
		<form method="post" action="options.php">
			<?php settings_fields( 'pdvwc_settings_group' ); ?>
			<?php do_settings_sections( 'pdvwc-options' ); ?>
			<table class="form-table">
				<tr valign="top">
					<th scope="row"><?php esc_html_e( 'Button Position', 'product-document-viewer-for-woocommerce' ); ?></th>
					<td>
						<select name="pdvwc_button_position">
							<?php
							$positions         = array(
							...
							$selected_position = get_option( 'pdvwc_button_position', 'woocommerce_single_product_summary' );
				...
				<tr valign="top">
					...
					<td>
						<input type="text" name="pdvwc_button_text" value="<?php echo esc_attr( get_option( 'pdvwc_button_text', 'Read a Little' ) ); ?>" />
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php esc_html_e( 'Extra Button Class', 'product-document-viewer-for-woocommerce' ); ?></th>
					<td>
						<input type="text" name="pdvwc_button_class" value="<?php echo esc_attr( get_option( 'pdvwc_button_class', '' ) ); ?>" />
				...
				<tr valign="top">
					...
					<td>
						<input type="color" name="pdvwc_button_color" value="<?php echo esc_attr( get_option( 'pdvwc_button_color', '#0073aa' ) ); ?>" />
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php esc_html_e( 'Hook Priority', 'product-document-viewer-for-woocommerce' ); ?></th>
					<td>
						<?php $current_priority = get_option( 'pdvwc_hook_priority', 30 ); ?>
						<select name="pdvwc_hook_priority">
...
function pdvwc_register_settings() {
	...
	// Color fields.
	pdvwc_add_color_field( 'pdvwc_button_font_color', __( 'Button Text Color', 'product-document-viewer-for-woocommerce' ), __( 'Select the button text color.', 'product-document-viewer-for-woocommerce' ) );
...
add_action( 'admin_init', 'pdvwc_register_settings', 20 );

// Checkbox field callback.
function pdvwc_checkbox_field_callback( $args ) {
	$option = get_option( $args['label_for'] );
	// Hidden fallback so an unchecked box still submits "0" instead of nothing,
	// letting the Settings API actually save the "off" state.
...
// Text field callback.
function pdvwc_text_field_callback( $args ) {
	$option = get_option( $args['label_for'] );
...
function pdvwc_color_field_callback( $args ) {
	$option = get_option( $args['label_for'] );
